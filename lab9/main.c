...
/* Get the least_access_slot accessed slot from TLB */
struct tlb_slot_t* get_least_access_slot_accessed_slot();
        ...
        for (int i = 0; i < PAGE_TABLE_SIZE; i++) {
            if (page_table[i].virtual == index) {
                ...
                *physical = ((page_table[i].physical << OFF_LEN) + offset);
                ...
                struct tlb_slot_t* least_access_slot = get_least_access_slot_accessed_slot();
                least_access_slot->virtual = page_table[i].virtual;
                least_access_slot->physical = page_table[i].physical;
                least_access_slot->count = 0;
                page_table[i].count++;
                exist = 1;
            }
        }
